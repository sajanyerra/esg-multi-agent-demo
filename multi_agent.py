...
@tool
def check_csrd_compliance(co2_metric_tons: float, threshold: float = 100.0) -> str:
    """Check if CO2 emissions exceed CSRD reporting threshold."""
    if co2_metric_tons > threshold:
        return f"⚠️ NON-COMPLIANT: {co2_metric_tons:.2f} tCO2 exceeds CSRD threshold of {threshold} tCO2. Reporting required."
    return f"✅ COMPLIANT: {co2_metric_tons:.2f} tCO2 is within CSRD threshold of {threshold} tCO2."
